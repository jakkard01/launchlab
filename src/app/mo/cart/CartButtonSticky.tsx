...
export default function CartButtonSticky({ onOpen }: CartButtonStickyProps) {
  ...
  if (totalItems === 0) {
    return (
      <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-0 right-0 z-50 flex justify-center px-4">
        <div className="flex w-full max-w-sm items-center justify-center rounded-full border border-default bg-surface px-5 py-3 text-xs font-semibold text-muted shadow-md">
  ...
  return (
    <div className="fixed bottom-[max(0.75rem,env(safe-area-inset-bottom))] left-0 right-0 z-50 flex justify-center px-4">
      <div className="flex w-full max-w-sm items-center justify-between gap-3 rounded-2xl border border-default bg-surface px-4 py-3 shadow-lg">
        ...
        <button
          type="button"
          onClick={onOpen}
          className="h-12 min-w-[44px] rounded-full bg-[var(--accent)] px-5 text-sm font-semibold text-[var(--surface)] transition hover:opacity-90"
